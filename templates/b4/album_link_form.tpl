...
<div class="card card-body bg-light m-1">
  <form action="index.php" method="post" id="myForm" enctype="multipart/form-data" role="form">
    <div class="form-group row">
      <div class="col-sm-12">
        <input type="text" name="link_url" id="LinkUrl" style="font-size:24px;" class="validate[required,custom[url]] form-control" placeholder="http://<{$smarty.const._MD_ALBUMLINK_LINK_URL}>" value="<{$link_url}>">
      </div>
    </div>

    <div class="form-group row">
      <div class="col-sm-6">
        <input type="text" name="link_title" id="LinkTitle" class="validate[required] form-control" placeholder="<{$smarty.const._MD_ALBUMLINK_LINK_TITLE}>" value="<{$link_title}>">
      </div>
      <div class="col-sm-4">
    ...
    <div class="form-group row">
      <{if $get_album_link_cate_options}>
        <div class="col-sm-6">
        ...
        <div class="col-sm-6">
          <{if $isAdmin}>
          <input type="text" name="new_cate" class="form-control" id="new_cate" placeholder="<{$smarty.const._MD_ALBUMLINK_NEW_CATE}>">
          <{/if}>
        </div>
      <{elseif $isAdmin}>
        <div class="col-sm-12">
         <input type="text" name="new_cate" class="validate[required] form-control" id="new_cate" placeholder="<{$smarty.const._MD_ALBUMLINK_ADD_NEW_CATE}>">
       </div>
      <{/if}>
    </div>
    ...
    <div class="form-group row">
      <div class="col-sm-6">
        <textarea name="link_desc" class="form-control" rows=3 id="LinkDesc" placeholder="<{$smarty.const._MD_ALBUMLINK_LINK_DESC}>"><{$link_desc}></textarea>
      </div>
      <div class="col-sm-2">
    ...
    <div class="form-group row">
      <div class="col-sm-12">
        ...
        <button type="submit" class="btn btn-info pull-right"><{if $link_sn}><{$smarty.const._TAD_SAVE}><{else}><{$smarty.const._MD_ALBUMLINK_QUICK_ADD}><{/if}></button>
      </div>
    </div>
